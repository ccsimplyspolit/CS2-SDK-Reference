...
Двуязычный быстрый старт. Развёрнуто — в [`wiki/`](wiki/).
...
переписывает только те файлы, у которых SHA-256 сменился, печатает
per-file diff.

### Сверка с живым cs2 (одной командой)

Нужен elevated shell (SeDebugPrivilege) и запущенный cs2.

```powershell
# Явный PID
python tools\verify_offsets.py --pid 53000

# Или автодетект + verify в одну команду
python tools\sync_from_upstream.py --verify
```
...
2. `python tools\verify_offsets.py --pid <cs2_pid>` — убедись, что новые оффсеты дереференсятся в правдоподобные pointer.
...
### Добавить новое маппинг поля (3 шага)
...
Открывай PR — каждое маппинг-поле должно нести source attribution (какой live-сэмпл / cs2-билд / PID).

---

## More / Дальше

- Deep dives: [`wiki/en/`](wiki/en/) / [`wiki/ru/`](wiki/ru/).
- Full CLI reference for the sync tool: [`wiki/en/07_sync_workflow.md`](wiki/en/07_sync_workflow.md) / [`wiki/ru/07_воркфлоу_sync.md`](wiki/ru/07_воркфлоу_sync.md).
- Runtime verification: [`wiki/en/08_runtime_verification.md`](wiki/en/08_runtime_verification.md) / [`wiki/ru/08_runtime_верификация.md`](wiki/ru/08_runtime_верификация.md).
- Glossary: [`wiki/en/10_glossary.md`](wiki/en/10_glossary.md) / [`wiki/ru/10_глоссарий.md`](wiki/ru/10_глоссарий.md).
