...
Human-readable summaries of the ~10 CS2 C++ classes any RE project keeps coming back to. Field offsets are current as of build 14169, verified field-by-field against the installed game on 2026-07-19 (values are byte-identical in 14170). See `schema/client_dll.json` for machine-readable canonical data.
...
Читаемые сводки по ~10 C++-классам CS2, к которым RE-проекты возвращаются постоянно. Оффсеты полей актуальны для билда 14169, сверены пополе с установленной игрой 2026-07-19 (в билде 14170 значения побайтово те же). Machine-readable canonical data — в `schema/client_dll.json`.
