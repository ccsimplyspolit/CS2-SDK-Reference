...
namespace source2sdk
{
    namespace client
    {
        ...
        {
        public:
            std::int32_t threshold_pct; // 0x1a78
            float bonus_attack_damage; // 0x1a7c
            float linger_duration; // 0x1a80
            std::int32_t min_bonus_pct; // 0x1a84
            std::int32_t max_bonus_pct; // 0x1a88
            float thirst_range; // 0x1a8c
        };
        #pragma pack(pop)

        // Cannot assert offsets of fields in CDOTA_Modifier_AntiMage_Mana_Thirst because it is not a standard-layout class

        static_assert(sizeof(source2sdk::client::CDOTA_Modifier_AntiMage_Mana_Thirst) == 0x1a90);
    };
};
