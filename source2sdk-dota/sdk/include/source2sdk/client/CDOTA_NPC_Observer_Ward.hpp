...
namespace source2sdk
{
    namespace client
    {
        ...
        {
        public:
            std::int32_t m_iDuration; // 0x19b8
            uint8_t _pad19bc[0x4]; // 0x19bc
            source2sdk::particleslib::CNewParticleEffect* m_pVisionRangeFX; // 0x19c0
            std::int32_t m_nPreviewViewer; // 0x19c8
            source2sdk::client::FowCustomTeams_t m_nFoWTeam; // 0x19cc
        };
        #pragma pack(pop)

        // Cannot assert offsets of fields in CDOTA_NPC_Observer_Ward because it is not a standard-layout class

        static_assert(sizeof(source2sdk::client::CDOTA_NPC_Observer_Ward) == 0x19d0);
    };
};
