...
#include "source2sdk/client/DOTANeutralCampInfo_t.hpp"
...
namespace source2sdk
{
    namespace client
    {
        ...
        public:
            ...
            // m_vecKnownCampStacks has a template type with potentially unknown template parameters. You can try uncommenting the field below.
            // C_UtlVectorEmbeddedNetworkVar<source2sdk::client::DOTANeutralCampInfo_t> m_vecKnownCampStacks;
            char m_vecKnownCampStacks[0x68]; // 0x18a0
            Vector2D m_vPossibleWardPlacement[100]; // 0x1908
            float m_vPossibleWardRadii[100]; // 0x1c28
            ...
            char m_vecTrackedTeleports[0x68]; // 0x1db8
            source2sdk::client::CRoshanPhaseInfo m_roshanSpawnInfo; // 0x1e20
            std::int32_t m_nNextPowerRuneType; // 0x1e38
            std::int32_t m_nNextPowerRuneSpawnIndex; // 0x1e3c
        };
        #pragma pack(pop)

        // Cannot assert offsets of fields in C_DOTA_DataNonSpectator because it is not a standard-layout class

        static_assert(sizeof(source2sdk::client::C_DOTA_DataNonSpectator) == 0x1e40);
    };
};
