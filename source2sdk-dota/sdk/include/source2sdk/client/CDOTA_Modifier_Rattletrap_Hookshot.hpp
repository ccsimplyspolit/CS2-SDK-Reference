...
namespace source2sdk
{
    namespace client
    {
        ...
        public:
            ...
            std::int32_t speed; // 0x1a8c
            float stun_radius; // 0x1a90
            float radius_ally; // 0x1a94
            float cooldown_refund_ally; // 0x1a98
            float damage; // 0x1a9c
            float duration; // 0x1aa0
            float ally_shield_duration; // 0x1aa4
        };
        #pragma pack(pop)

        // Cannot assert offsets of fields in CDOTA_Modifier_Rattletrap_Hookshot because it is not a standard-layout class

        static_assert(sizeof(source2sdk::client::CDOTA_Modifier_Rattletrap_Hookshot) == 0x1aa8);
    };
};
