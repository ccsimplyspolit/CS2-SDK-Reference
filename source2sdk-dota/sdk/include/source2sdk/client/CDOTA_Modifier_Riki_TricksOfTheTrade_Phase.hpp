...
namespace source2sdk
{
    namespace client
    {
        ...
        public:
            ...
            float attack_damage; // 0x1a80            
            std::int32_t agility_pct; // 0x1a84            
            float cooldown_reduction_per_creep_kill; // 0x1a88            
            float speed_per_attack; // 0x1a8c            
            float radius; // 0x1a90            
            std::int32_t interval_targets; // 0x1a94            
            bool pocket_riki_enabled; // 0x1a98            
            uint8_t _pad1a99[0x3]; // 0x1a99
            float base_attack_damage_pct; // 0x1a9c            
        };
        #pragma pack(pop)
        
        // Cannot assert offsets of fields in CDOTA_Modifier_Riki_TricksOfTheTrade_Phase because it is not a standard-layout class
        
        static_assert(sizeof(source2sdk::client::CDOTA_Modifier_Riki_TricksOfTheTrade_Phase) == 0x1aa0);
    };
};
