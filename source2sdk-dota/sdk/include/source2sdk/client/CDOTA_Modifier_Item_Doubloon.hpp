...
namespace source2sdk
{
    namespace client
    {
        ...
        {
        public:
            float conversion_pct; // 0x1a78
            std::int32_t bonus_health_regen; // 0x1a7c
            float bonus_mana_regen; // 0x1a80
            std::int32_t regen_bonus_pct; // 0x1a84
        };
        #pragma pack(pop)

        // Cannot assert offsets of fields in CDOTA_Modifier_Item_Doubloon because it is not a standard-layout class

        static_assert(sizeof(source2sdk::client::CDOTA_Modifier_Item_Doubloon) == 0x1a88);
    };
};
