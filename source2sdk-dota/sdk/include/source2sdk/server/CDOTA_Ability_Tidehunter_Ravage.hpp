...
namespace source2sdk
{
    namespace server
    {
        ...
        public:
            ...
            char m_hEntsHit[0x18]; // 0x580            
            bool m_bAwardedKillEater; // 0x598            
            uint8_t _pad0599[0x3]; // 0x599
            float duration; // 0x59c            
        };
        #pragma pack(pop)
        
        // Cannot assert offsets of fields in CDOTA_Ability_Tidehunter_Ravage because it is not a standard-layout class
        
        static_assert(sizeof(source2sdk::server::CDOTA_Ability_Tidehunter_Ravage) == 0x5a0);
    };
};
