...
namespace source2sdk
{
    namespace server
    {
        ...
        {
        public:
            std::int32_t damage_per_unit; // 0x580            
            float radius; // 0x584            
            std::int32_t max_units; // 0x588            
            std::int32_t tombstone_heal; // 0x58c            
        };
        #pragma pack(pop)
        
        // Cannot assert offsets of fields in CDOTA_Ability_Undying_SoulRip because it is not a standard-layout class
        
        static_assert(sizeof(source2sdk::server::CDOTA_Ability_Undying_SoulRip) == 0x590);
    };
};
