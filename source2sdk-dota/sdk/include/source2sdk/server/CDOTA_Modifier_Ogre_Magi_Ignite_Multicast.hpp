...
namespace source2sdk
{
    namespace server
    {
        ...
        public:
            ...
            float multicast_delay; // 0x1a7c
            std::int32_t m_nMultiCastCount; // 0x1a80
            float ignite_multicast_aoe; // 0x1a84
        };
        #pragma pack(pop)

        // Cannot assert offsets of fields in CDOTA_Modifier_Ogre_Magi_Ignite_Multicast because it is not a standard-layout class

        static_assert(sizeof(source2sdk::server::CDOTA_Modifier_Ogre_Magi_Ignite_Multicast) == 0x1a88);
    };
};
