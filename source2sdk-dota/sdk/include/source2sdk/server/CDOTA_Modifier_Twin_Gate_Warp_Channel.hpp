...
namespace source2sdk
{
    namespace server
    {
        ...
        public:
            ...
            char m_hOtherPortal[0x4]; // 0x1a90
            VectorWS m_vStartPosition; // 0x1a94
            float m_flTotalTime; // 0x1aa0
            float m_flElapsedTimePortion; // 0x1aa4
            float animation_rate; // 0x1aa8
            std::int32_t stop_distance; // 0x1aac
        };
        #pragma pack(pop)

        // Cannot assert offsets of fields in CDOTA_Modifier_Twin_Gate_Warp_Channel because it is not a standard-layout class

        static_assert(sizeof(source2sdk::server::CDOTA_Modifier_Twin_Gate_Warp_Channel) == 0x1ab0);
    };
};
