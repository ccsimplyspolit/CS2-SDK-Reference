...
namespace source2sdk
{
    namespace server
    {
        ...
        {
        public:
            float m_flSongDuration; // 0x1a78
            float m_flSongStartTime; // 0x1a7c
            float mana_pct; // 0x1a80
            float animation_rate; // 0x1a84
        };
        #pragma pack(pop)

        // Cannot assert offsets of fields in CDOTA_Modifier_NagaSiren_SongOfTheSiren because it is not a standard-layout class

        static_assert(sizeof(source2sdk::server::CDOTA_Modifier_NagaSiren_SongOfTheSiren) == 0x1a88);
    };
};
