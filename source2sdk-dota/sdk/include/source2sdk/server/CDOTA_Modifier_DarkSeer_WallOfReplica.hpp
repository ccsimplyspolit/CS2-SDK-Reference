...
namespace source2sdk
{
    namespace server
    {
        ...
        public:
            ...
            char m_PreventReplicateTime[0x28]; // 0x1aa8
            float width; // 0x1ad0
            std::int32_t wall_damage; // 0x1ad4
            float slow_duration; // 0x1ad8
            std::int32_t replica_damage_incoming; // 0x1adc
            std::int32_t replica_damage_outgoing; // 0x1ae0
            uint8_t _pad1ae4[0x14];
        };
        #pragma pack(pop)

        // Cannot assert offsets of fields in CDOTA_Modifier_DarkSeer_WallOfReplica because it is not a standard-layout class

        static_assert(sizeof(source2sdk::server::CDOTA_Modifier_DarkSeer_WallOfReplica) == 0x1af8);
    };
};
