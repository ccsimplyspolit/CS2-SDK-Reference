...
namespace source2sdk
{
    namespace server
    {
        ...
        {
        public:
            float snare_radius; // 0x1a78
            float snare_duration; // 0x1a7c
            VectorWS m_vLastPosition; // 0x1a80
            // m_hLastThinker has a template type with potentially unknown template parameters. You can try uncommenting the field below.
            // CHandle<source2sdk::server::CBaseEntity> m_hLastThinker;
            char m_hLastThinker[0x4]; // 0x1a8c
        };
        #pragma pack(pop)

        // Cannot assert offsets of fields in CDOTA_Modifier_Broodmother_WebWalk because it is not a standard-layout class

        static_assert(sizeof(source2sdk::server::CDOTA_Modifier_Broodmother_WebWalk) == 0x1a90);
    };
};
