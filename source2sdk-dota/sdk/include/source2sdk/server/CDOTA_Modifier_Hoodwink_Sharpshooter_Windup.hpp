...
namespace source2sdk
{
    namespace server
    {
        ...
        public:
            ...
            char m_vecVisionThinkers[0x18]; // 0x1a78            
            float arrow_vision; // 0x1a90            
            float max_charge_time; // 0x1a94            
            float turn_rate; // 0x1a98            
            float base_power; // 0x1a9c            
        };
        #pragma pack(pop)
        
        // Cannot assert offsets of fields in CDOTA_Modifier_Hoodwink_Sharpshooter_Windup because it is not a standard-layout class
        
        static_assert(sizeof(source2sdk::server::CDOTA_Modifier_Hoodwink_Sharpshooter_Windup) == 0x1aa0);
    };
};
