...
namespace source2sdk
{
    namespace server
    {
        ...
        public:
            ...
            char m_vecAbilities[0x18]; // 0x1a78
            float m_fNextCastTime; // 0x1a90
            float m_fCastInterval; // 0x1a94
            float m_fWarningTime; // 0x1a98
            std::int32_t m_iNextAbility; // 0x1a9c
        };
        #pragma pack(pop)

        // Cannot assert offsets of fields in CDOTA_Modifier_Mutation_Spellcast because it is not a standard-layout class

        static_assert(sizeof(source2sdk::server::CDOTA_Modifier_Mutation_Spellcast) == 0x1aa0);
    };
};
