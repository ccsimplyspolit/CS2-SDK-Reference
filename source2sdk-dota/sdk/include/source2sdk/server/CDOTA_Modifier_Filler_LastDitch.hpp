...
namespace source2sdk
{
    namespace server
    {
        ...
        {
        public:
            float melee_number; // 0x1a78
            float ranged_number; // 0x1a7c
            float siege_number; // 0x1a80
            std::int32_t lane; // 0x1a84
        };
        #pragma pack(pop)

        // Cannot assert offsets of fields in CDOTA_Modifier_Filler_LastDitch because it is not a standard-layout class

        static_assert(sizeof(source2sdk::server::CDOTA_Modifier_Filler_LastDitch) == 0x1a88);
    };
};
