...
#include "source2sdk/server/DOTANeutralCampInfo_t.hpp"
...
namespace source2sdk
{
    namespace server
    {
        ...
        public:
            ...
            // CUtlVectorEmbeddedNetworkVar<source2sdk::server::DOTANeutralCampInfo_t> m_vecKnownCampStacks;
            char m_vecKnownCampStacks[0x68]; // 0x1c58
            source2sdk::server::CRoshanPhaseInfo m_roshanSpawnInfo; // 0x1cc0
            std::int32_t m_nNextPowerRuneType; // 0x1cd8
            std::int32_t m_nNextPowerRuneSpawnIndex; // 0x1cdc
            std::int32_t m_iTowerKills; // 0x1ce0
            uint8_t _pad1ce4[0x4];
        };
        #pragma pack(pop)

        // Cannot assert offsets of fields in CDOTA_DataNonSpectator because it is not a standard-layout class

        static_assert(sizeof(source2sdk::server::CDOTA_DataNonSpectator) == 0x1ce8);
    };
};
