...
namespace source2sdk
{
    namespace server
    {
        ...
        {
        public:
            float move_speed_per_instance; // 0x1a78
            std::int32_t attack_speed_per_instance; // 0x1a7c
            char* m_pSlotName; // 0x1a80
        };
        #pragma pack(pop)

        // Cannot assert offsets of fields in CDOTA_Modifier_Invoker_WexInstance because it is not a standard-layout class

        static_assert(sizeof(source2sdk::server::CDOTA_Modifier_Invoker_WexInstance) == 0x1a88);
    };
};
