...
namespace source2sdk
{
    namespace server
    {
        ...
        {
        public:
            std::int32_t m_nTeamNumber; // 0x1a78
            std::int32_t m_iCapturingTeam; // 0x1a7c
            float m_flCaptureProgress; // 0x1a80
            float m_flBonusTime; // 0x1a84
            bool m_bInitialSetupDone; // 0x1a88
            uint8_t _pad1a89[0x3]; // 0x1a89
            std::int32_t m_iOriginalTeam; // 0x1a8c
            ...
            char m_hPreviousHero[0x4]; // 0x1a90
            ...
            char m_vecLastCreditedPlayerIDs[0x18]; // 0x1a98
            float m_flAccumulatedCaptureTime; // 0x1ab0
            std::int32_t m_iBonusCount; // 0x1ab4
            source2sdk::client::ParticleIndex_t m_nFxOutpostAmbient; // 0x1ab8
            source2sdk::client::ParticleIndex_t m_nFxOutpostInitialAmbient; // 0x1abc
        };
        #pragma pack(pop)

        // Cannot assert offsets of fields in CDOTA_Modifier_Watch_Tower because it is not a standard-layout class

        static_assert(sizeof(source2sdk::server::CDOTA_Modifier_Watch_Tower) == 0x1ac0);
    };
};
