...
namespace source2sdk
{
    namespace server
    {
        ...
        public:
            ...
            source2sdk::server::CountdownTimer m_Timer; // 0x1a80
            ...
            float attack_rate; // 0x1aa4
            float attack_damage; // 0x1aa8
            std::int32_t agility_pct; // 0x1aac
            float cooldown_reduction_per_creep_kill; // 0x1ab0
            float speed_per_attack; // 0x1ab4
            float radius; // 0x1ab8
            std::int32_t interval_targets; // 0x1abc
            bool pocket_riki_enabled; // 0x1ac0
            uint8_t _pad1ac1[0x3]; // 0x1ac1
            float base_attack_damage_pct; // 0x1ac4
        };
        #pragma pack(pop)

        // Cannot assert offsets of fields in CDOTA_Modifier_Riki_TricksOfTheTrade_Phase because it is not a standard-layout class

        static_assert(sizeof(source2sdk::server::CDOTA_Modifier_Riki_TricksOfTheTrade_Phase) == 0x1ac8);
    };
};
