...
namespace source2sdk
{
    namespace server
    {
        ...
        {
        public:
            std::int32_t base_attack_damage; // 0x1a78
            std::int32_t magic_resistance; // 0x1a7c
            std::int32_t status_resistance; // 0x1a80
            float attack_speed; // 0x1a84
        };
        #pragma pack(pop)

        // Cannot assert offsets of fields in CDOTA_Modifier_Item_Titan_Sliver because it is not a standard-layout class

        static_assert(sizeof(source2sdk::server::CDOTA_Modifier_Item_Titan_Sliver) == 0x1a88);
    };
};
