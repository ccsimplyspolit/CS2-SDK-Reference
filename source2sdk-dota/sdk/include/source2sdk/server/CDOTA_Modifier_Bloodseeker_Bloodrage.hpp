...
namespace source2sdk
{
    namespace server
    {
        ...
        {
        public:
            std::int32_t attack_speed; // 0x1a78
            std::int32_t spell_amp; // 0x1a7c
            float max_health_dmg_pct; // 0x1a80
            float damage_pct; // 0x1a84
            float base_damage_amp; // 0x1a88
            float flTickRate; // 0x1a8c
        };
        #pragma pack(pop)

        // Cannot assert offsets of fields in CDOTA_Modifier_Bloodseeker_Bloodrage because it is not a standard-layout class

        static_assert(sizeof(source2sdk::server::CDOTA_Modifier_Bloodseeker_Bloodrage) == 0x1a90);
    };
};
