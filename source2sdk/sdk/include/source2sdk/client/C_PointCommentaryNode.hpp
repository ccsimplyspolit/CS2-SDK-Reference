#pragma once

#include "source2sdk/source2gen/source2gen.hpp"
#include <cstddef>
#include <cstdint>
#include "source2sdk/client/CBaseAnimGraph.hpp"
#include "source2sdk/entity2/GameTime_t.hpp"
namespace source2sdk
{
    namespace client
    {
        struct CSoundPatch;
    };
};
namespace source2sdk
{
    namespace client
    {
        struct C_BaseEntity;
    };
};

// /////////////////////////////////////////////////////////////
// Module: client
// Created using source2gen - github.com/neverlosecc/source2gen
// /////////////////////////////////////////////////////////////

namespace source2sdk
{
    namespace client
    {
        // Registered alignment: 0x8
        // Alignment: 0x8
        // Standard-layout class: false
        // Size: 0x11e0
        // Has VTable
        // Construct allowed
        #pragma pack(push, 1)
        class C_PointCommentaryNode : public source2sdk::client::CBaseAnimGraph
        {
        public:
            uint8_t _pad1180[0x18]; // 0x1180
            bool m_bActive; // 0x1198            
            bool m_bWasActive; // 0x1199            
            uint8_t _pad119a[0x2]; // 0x119a
            source2sdk::entity2::GameTime_t m_flEndTime; // 0x119c            
            source2sdk::entity2::GameTime_t m_flStartTime; // 0x11a0            
            float m_flStartTimeInCommentary; // 0x11a4            
            CUtlSymbolLarge m_iszCommentaryFile; // 0x11a8            
            CUtlSymbolLarge m_iszTitle; // 0x11b0            
            CUtlSymbolLarge m_iszSpeakers; // 0x11b8            
            std::int32_t m_iNodeNumber; // 0x11c0            
            std::int32_t m_iNodeNumberMax; // 0x11c4            
            bool m_bListenedTo; // 0x11c8            
            uint8_t _pad11c9[0x7]; // 0x11c9
            source2sdk::client::CSoundPatch* m_sndCommentary; // 0x11d0            
            // m_hViewPosition has a template type with potentially unknown template parameters. You can try uncommenting the field below.
            // CHandle<source2sdk::client::C_BaseEntity> m_hViewPosition;
            char m_hViewPosition[0x4]; // 0x11d8            
            // metadata: MNotSaved
            bool m_bRestartAfterRestore; // 0x11dc            
            uint8_t _pad11dd[0x3];
        };
        #pragma pack(pop)
        
        // Cannot assert offsets of fields in C_PointCommentaryNode because it is not a standard-layout class
        
        static_assert(sizeof(source2sdk::client::C_PointCommentaryNode) == 0x11e0);
    };
};
